#!/usr/bin/env python
#
# script to plot r-value for all pairs of variables on separate plots. No comparison with atlas results.
...
infile = 'collective_results_minigrid.txt'
...
labels2 = ['G', 'Q', 'Q3', 'N1']
data_raw = np.loadtxt(infile, skiprows=1)
...
def plot(xar, xlab, xlab2, yar, ylab, ylab2, zar):
	# declare plot
	fig = plt.figure()
	...
	# axes labels
	ax.set_xlabel(xlab+r'$ [\rm GeV]$', fontsize=23)
	ax.set_ylabel(ylab+r'$ [\rm GeV]$', fontsize=23)
	# axes ranges
	...
	# scatter plot
	vmin, vmax = 10**-2, 10
	sc = ax.scatter(xar, yar, s=50, c=zar, cmap='nipy_spectral', norm=cls.LogNorm(), 
		vmin=vmin, vmax=vmax, lw=0.5, edgecolors='none', marker='o', alpha=0.7, rasterized=False)    
	# color bar
	cb = plt.colorbar(sc)        
	...
	tag = infile.split('.')[0]
	plot_name = plot_prefix+'_'+xlab2+'_'+ylab2
...
def make_plots(data, labels):
	for ii in range(len(data[:-1])):
		for jj in range(ii+1, len(data[:-1])):
			print(ii, jj)
			plot(data[ii], labels[ii], labels2[ii], data[jj], labels[jj], labels2[jj], data[-1])
